...
static void start_recording_video(std::string file_path, GstElement *pipe1) {
    g_print("start_recording_video file \n");
...
gboolean RtspPipelineHandler::start_streaming() {
    ...
    if (FROM_PCAP) {
        ...
        // Below for  video test src
        /*pipeline_string = string("tee name=videotee allow-not-linked=true ") +
                          string("videotestsrc num-buffers=-1 ! video/x-raw, format=I420, width=320, height=240, framerate=25/1 ! x264enc ! videotee. ");
        */
    }
    ...
    //file_name = prepare_next_file_name();
    start_recording_video(file_name, pipeline); //Need to check this
